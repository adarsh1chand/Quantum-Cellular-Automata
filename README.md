...
- Left click on a cell repeatedly, to increase the "redness" (Red color value or the shade of Red) of the cell, once the maximum value of 1 is reached, prolonged left clicking leads to decrease in the "redness". In all versions, whether it is vanilla version, continuous or quantum version user can only change the "redness" of a cell before starting the evolution. In the vanilla version, a red cell with Red color value > 127 is treated as *alive* cell. In the continuous version, a cell can take different 'shades' of red i.e., the Red color value can be any integer between (0 - 255). Finally, in the quantum version, during evolution, a cell can take 8 different colors corresponding to 8 possible configurations of a cell's state or equivalently, each of the RGB values for the color of a cell can be either 0 or 255 (see the section **Quantum version** above).
